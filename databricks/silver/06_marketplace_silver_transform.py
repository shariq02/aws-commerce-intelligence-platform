...
marketplace_flat = spark.table(TARGET_TABLE) \
    ...
    .select(
        ...
        F.coalesce(F.get_json_object(F.col("payload"), "$.seller_tier"), F.lit("unknown")).alias("seller_tier"),
        ...
        F.coalesce(F.get_json_object(F.col("payload"), "$.seller_region"), F.lit("unknown")).alias("seller_region"),
        ...
        F.coalesce(F.get_json_object(F.col("payload"), "$.is_sla_breached").cast("boolean"), F.lit(False)).alias("is_sla_breached"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.dispatch_speed_bucket"), F.lit("unknown")).alias("dispatch_speed_bucket"),
