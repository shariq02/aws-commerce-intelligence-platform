...
pharmacy_flat = spark.table(TARGET_TABLE) \
    ...
    .select(
        ...
        F.coalesce(F.get_json_object(F.col("payload"), "$.category"), F.lit("unknown")).alias("category"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.category_group"), F.lit("unknown")).alias("category_group"),
        F.get_json_object(F.col("payload"), "$.atc_code").alias("atc_code"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.drug_class"), F.lit("unknown")).alias("drug_class"),
        F.get_json_object(F.col("payload"), "$.quantity").cast("double").alias("quantity"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.is_prescription").cast("boolean"), F.lit(False)).alias("is_prescription"),
        ...
        F.coalesce(F.get_json_object(F.col("payload"), "$.weekday"), F.lit("unknown")).alias("weekday"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.is_weekend").cast("boolean"), F.lit(False)).alias("is_weekend"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.time_of_day"), F.lit("unknown")).alias("time_of_day"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.is_business_hours").cast("boolean"), F.lit(False)).alias("is_business_hours"),
        F.coalesce(F.get_json_object(F.col("payload"), "$.is_peak_hour").cast("boolean"), F.lit(False)).alias("is_peak_hour"),
