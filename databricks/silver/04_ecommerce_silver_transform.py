...
ecommerce_flat = all_events.select(
    ...
    F.coalesce(F.get_json_object(F.col("payload"), "$.customer_segment"), F.lit("unknown")).alias("customer_segment"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.order_value_tier"), F.lit("unknown")).alias("order_value_tier"),
    F.get_json_object(F.col("payload"), "$.region").alias("region"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.state_region"), F.lit("unknown")).alias("state_region"),
    ...
    F.coalesce(F.get_json_object(F.col("payload"), "$.payment_method"), F.lit("unknown")).alias("payment_method"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.is_installment").cast("boolean"), F.lit(False)).alias("is_installment"),
    F.get_json_object(F.col("payload"), "$.max_installments").cast("int").alias("max_installments"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.order_status"), F.lit("unknown")).alias("order_status"),
    F.get_json_object(F.col("payload"), "$.item_count").cast("int").alias("item_count"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.is_multi_item").cast("boolean"), F.lit(False)).alias("is_multi_item"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.is_multi_seller").cast("boolean"), F.lit(False)).alias("is_multi_seller"),
    ...
    F.coalesce(F.get_json_object(F.col("payload"), "$.fulfilment_bucket"), F.lit("unknown")).alias("fulfilment_bucket"),
    ...
    F.coalesce(F.get_json_object(F.col("payload"), "$.review_sentiment"), F.lit("unknown")).alias("review_sentiment"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.has_negative_review").cast("boolean"), F.lit(False)).alias("has_negative_review"),
    F.coalesce(F.get_json_object(F.col("payload"), "$.return_reason"), F.lit("none")).alias("return_reason"),
)
